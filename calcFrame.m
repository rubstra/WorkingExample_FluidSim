function [ frame, fluidPixels ] = calcFrame(...
    visualSettings,...
    distanceField,...
    obstacle,...
    velocity,...
...
%NOTE: THIS MATLAB FUNCTION IS TAKEN FROM THE WORK OF PHILIP ERLER WITHOUT
%CHANGES. THIS CAN BE CONSIDERED AS THE POST PROCESSOR FOR VISUAL
%PRESENTATION OF DATA. THE FOCUS OF THIS WORK HAS NOT BEEN DIRECTD TOWARDS
%THIS PART AND FOR THIS REASON, THE FUNCTION WILL NOT BE EXPLAINED STEP BY
%STEP, AS OTHER FUNCTIONS, BUT THE ORIGINAL COMMENTS FROM ERLER WILL BE
%KEPT.
...
%   pressureField: matrix(yNodes, xNodes)
    ...
    gridCellSize = visualSettings.gridCellSize;
    ...
    distanceSteps = double(int32(abs(inflatedDistanceField))) ./ ...
        visualSettings.isobarWidth;
    % limit to n isobars
    maxDistance = 10 ./ visualSettings.isobarWidth;
    ...
    waterColorFactors = [0.4 0.7 0.9];
    airColorFactors = [0.99 0.99 0.99];
    ...
    % draw pressure
    if visualSettings.drawPressure
    ...
    if visualSettings.drawVelocity
